...
pub async fn get_books_by_category_id(
    ...
    let category_name = match crate::api::repo::categories::get_category_name_by_id(&state.db_pool, category_id).await {
        Ok(Some(name)) => name,
    ...
    // 2. On formate la requête pour Google Books au format subject:NomDeLaCategorie
    let google_query = format!("subject:{}", category_name);
